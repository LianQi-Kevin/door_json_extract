""" Step.0 - 从 pdf 文件中截取信息表, 保存到 <config.cache_png_dir> / <pdf filename>.png """
import logging
...
DOOR_DRAW_BASE_DIR: Path = Path(r"防火门深化图")
...
if __name__ == '__main__':
    log_set(logging.DEBUG, log_save=True, save_level=logging.WARNING, save_path=config.log_dir / "0_corp_pdf_to_png.log")

    for index, pdf_path in enumerate(get_door_drawing_path(DOOR_DRAW_BASE_DIR, ["FHM", "GM"]), start=1):
        logging.debug(f"{index} {pdf_path}")
        croped_png = crop_pdf_region_to_png(pdf_path, ((0.62, 0.53), (0.87, 0.98)), dpi=400)
